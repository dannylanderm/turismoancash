...
?>
<table id='tblsitio' class='table table-responsive txt_left'>
    <tr>
        <th>ID</th>
        <th>Nombre</th>
        <th>Tipo de sitio</th>
	...
	<?php foreach ($sitio_list as $row) { ?>
        <tr>
            ...
            <td class='txt_right'> <?php echo DECtoDMS($row['sitio_latitud_geo'], true); ?></td>
            <td class='txt_right'> <?php echo DECtoDMS($row['sitio_longitud_geo'], true); ?></td>
            ...
            <td hidden><?php echo $row['sitio_calificacion']; ?></td>
            <td hidden><?php echo $row['sitio_situacion']; ?></td>
            <td hidden><?php echo $row['sitio_estado']; ?></td>
            <td class='txt_center'>
                <a href='#' class='btn btn-info' onclick="sitio_editar('<?php echo $row['sitio_id']; ?>');"><img
                            src='../resources/img/edit.png' style='width: 16px;'></a></td>
            <td class='txt_center'>
                <a href='#' class='btn btn-danger' onclick="sitio_borrar('<?php echo $row['sitio_id']; ?>');"><img
...
<script>
    function sitio_editar(sitio_id) {
        performLoad('sitio/sitioReg.php?sitio_id=' + sitio_id);
    }
