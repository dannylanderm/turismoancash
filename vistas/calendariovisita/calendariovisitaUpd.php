...
<table class='form_data'>
	<tr><td><label for='txtCalendLugID'>Lugar turístico:</label></td>
		<td><select class='form-control txt200' id='txtCalendLugID' name='txtCalendLugID'> <!-- maxlength='10' -->
			...
			<?php $lug_list = $lug_dal->listarcbo($calend_row['calend_lug_id']);  foreach($lug_list as $row) { ?>
	...
	<tr><td><label for='txtCalendNro'>Nro:</label></td>
		<td><input type='text' class='form-control txt200' id='txtCalendNro' name='txtCalendNro' value='<?php if ($calend_row) { echo $calend_row['calend_nro']; } ?>' maxlength='10' placeholder='Ingrese nro'/></td>
	</tr>
	<tr><td><label for='txtCalendFechaIni'>Fecha ini:</label></td>
		<td><input type='text' class='form-control txt200' id='txtCalendFechaIni' name='txtCalendFechaIni' value='<?php if ($calend_row) { echo formatDate($calend_row['calend_fecha_ini']); } ?>'  placeholder='Ingrese fecha ini'/></td>
	</tr>
	<tr><td><label for='txtCalendFechaFin'>Fecha fin:</label></td>
		<td><input type='text' class='form-control txt200' id='txtCalendFechaFin' name='txtCalendFechaFin' value='<?php if ($calend_row) { echo formatDate($calend_row['calend_fecha_fin']); } ?>'  placeholder='Ingrese fecha fin'/></td>
	</tr>
	<tr><td><label for='txtCalendHoraIni'>Hora ini:</label></td>
		<td><input type='text' class='form-control txt200' id='txtCalendHoraIni' name='txtCalendHoraIni' value='<?php if ($calend_row) { echo $calend_row['calend_hora_ini']; } ?>'  placeholder='Ingrese hora ini'/></td>
	</tr>
	<tr><td><label for='txtCalendHoraFin'>Hora fin:</label></td>
		<td><input type='text' class='form-control txt200' id='txtCalendHoraFin' name='txtCalendHoraFin' value='<?php if ($calend_row) { echo $calend_row['calend_hora_fin']; } ?>'  placeholder='Ingrese hora fin'/></td>
	</tr>
	<tr><td><label for='txtCalendSituacion'>Situacion:</label></td>
		<td><input type='text' class='form-control txt200' id='txtCalendSituacion' name='txtCalendSituacion' value='<?php if ($calend_row) { echo $calend_row['calend_situacion']; } ?>'  placeholder='Ingrese situacion'/></td>
	</tr>
	<tr hidden><td><label for='txtCalendEstado'>Estado:</label></td>
		<td><input type='text' class='form-control txt200' id='txtCalendEstado' name='txtCalendEstado' value='<?php if ($calend_row) { echo $calend_row['calend_estado']; } ?>'  placeholder='Ingrese estado'/></td>
	</tr>
</table>
<hr class='separator'/>
<div class='form_foot'>
