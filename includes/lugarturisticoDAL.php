<?php
	...
	class lugarturisticoDAL
	{
		...
		function getSitiosCercanos($lug_id, $b) {
			$mysql = new Conexion();
			$rs    = $mysql->ejecutar("CALL pa_lugarturistico_getSitiosCercanos('$lug_id', '$b');");
			return $mysql->rsToArray($rs);
		}
		
		function getActividades($lug_id, $b) {
			$mysql = new Conexion();
			$rs    = $mysql->ejecutar("CALL pa_lugarturistico_getActividades('$lug_id', '$b');");
			return $mysql->rsToArray($rs);
		}
		
		function getRecomendaciones($lug_id, $b) {
			$mysql = new Conexion();
			$rs    = $mysql->ejecutar("CALL pa_lugarturistico_getRecomendaciones('$lug_id', '$b');");
			return $mysql->rsToArray($rs);
		}
		
		function getObjetosTuristicos($lug_id, $b) {
			$mysql = new Conexion();
			$rs    = $mysql->ejecutar("CALL pa_lugarturistico_getObjetosTuristicos('$lug_id', '$b');");
			return $mysql->rsToArray($rs);
		}
